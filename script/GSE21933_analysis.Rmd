...
gsg$allOK #IF TRUE all the values are OK. 
gsg$goodGenes 
gsg$goodSamples # all samples are OK (42 of 42)
```
...
sfree <- ggplot(sft_df,aes(x = Power, y = SignedR2, label = Power))+
  ...
  geom_vline(xintercept = 10, linetype= "dashed", color = "red",linewidth=0.6)+
...
mfree <- ggplot(sft_df, aes(x = Power, y = MeanConnectivity, label=Power))+
  ...
  geom_hline(yintercept = 10, linetype= "dashed", color = "#300999",linewidth=1)+
  geom_vline(xintercept = 10, linetype= "dashed", color = "red",linewidth=0.6)+
  theme_pubr()

ggarrange(plotlist = list(sfree,mfree), nrow = 2,ncol = 1)
ggsave(filename="WGCNA_Threshold.png", path = "../figures")

chosen_threshold <- sft_df$Power[10]
mean_conectivity <- sft_df$MeanConnectivity[chosen_threshold]

```
I chose the threshold to be at 10 as this was falling into 0.8 to 0.9 range on my Scale-Free Topology Fit Index as well as my mean connectivity was `r mean_conectivity`.
