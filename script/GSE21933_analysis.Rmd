...
for(m in modules){
  ...
  hub_genes <- module_genes[order(kWithin[module_genes, "kWithin"], decreasing = TRUE)]# Order genes by connectivity
  hup_genes <- featuresinfo[hub_genes, "GENE_SYMBOL"]
  write_lines(hup_genes, file = paste0("../outputs/hub_",m,".txt"))
