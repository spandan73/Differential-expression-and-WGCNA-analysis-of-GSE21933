...
First I am filtering out only genes that belong to group-6 in the array as that represents the control genes. Lowly expressed genes must be filtered out as they can deviate the results of gene expression. Here I am applying three separate
strategies: All samples have a data, at least 50 percent have a data, and 2 or more samples have a data. 
...
keep <- rowSums(is_expressed)>2 # linent present in atleast 2 samples
...
```{r}
set.seed(1234)
design_gse_combined <- model.matrix(~histology+tissue, data = samplesinfo)
design_gse_interaction <- model.matrix(~0+factor(samplesinfo$tissue)*factor(samplesinfo$histology)*factor(samplesinfo$stage))
colnames(design_gse_combined)<-  c("tumor","normal","Neg","SQ","IB","IIB","IIIA","IIIB","IV","Neg_stage") 
